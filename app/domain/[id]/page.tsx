...
export default async function DomainPage({ params, searchParams }: { params: Promise<{ id: string }>, searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  const advancedDomains = ['o2c-b2b', 'o2c-b2c', 'p2p-import', 'p2p-assets', 'plan-to-produce', 'hr-recruit'];
  const showLegend = advancedDomains.includes(resolvedParams.id);
  ...
  const header = (
    <>
      ...
      <h1 className="text-4xl font-bold tracking-tight text-primary mb-2 capitalize">
        {resolvedParams.id.replace(/-/g, ' ')} Roadmap
      </h1>
      ...
      <div className="flex justify-between items-center mb-6 pr-4">
         ...
         <div className="flex items-center gap-2">
           ...
           <Link href={`/domain/${resolvedParams.id}`} className="p-2 hover:bg-node-bg border border-node-border rounded-md text-foreground transition-colors">
             <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" x2="6" y1="20" y2="6"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
           </Link>
         </div>
      </div>
  ...
  return (
    <main className="flex min-h-screen items-start justify-center p-6 lg:p-12 bg-background relative">
      <RoadmapLayout 
        domainId={resolvedParams.id} 
        initialNodes={mapData.nodes} 
        initialEdges={mapData.edges} 
        nodeId={nodeId}
        mdxContent={mdxElement}
        header={header}
